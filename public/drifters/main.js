...
class DrifterApp {
  ...
  drawDrifters(data, drawdate, with_tx) {
    ...
    for (let [name, path] of Object.entries(data)) {
      ...
      let src = 'drifters/marker.svg'
      if (hourdiff > 24 || with_tx == false) {
        src = 'drifters/marker_notx.svg'
      }
  ...
  setupSocialButtons() {
    $('.linkedin')[0].onclick = (e) =>
      window.open(
        `https://www.linkedin.com/shareArticle?mini=true&url=${this.generateQueryURL(true)}`,
      )
    $('.facebook')[0].onclick = (e) =>
      window.open(
        `https://www.facebook.com/sharer.php?u=${this.generateQueryURL(true)}&t=${this.generateQueryURL(true)}`,
      )
